...
- Your installed CUDA™ Toolkit is **too old** to support your GPU. Support for each compute capability is added in a specific toolkit version (for example, `compute_120` requires CUDA™ 12.8 or newer). With a brand-new GPU, the fix is often to **upgrade the CUDA™ Toolkit**, not to change the flag.
...
This corresponds to compute capability **8.9** (as shown in the [architecture table below](#recent-architectures)). Newer GPUs print three digits, e.g. `110` for a Jetson™ AGX Thor (11.0) or `120` for a GeForce RTX 5090 (12.0).
...
You can also build and run the `deviceQuery` program from the [CUDA™ samples](https://github.com/NVIDIA/cuda-samples){: target="_blank"}, which prints the compute capability together with other device details.

#### Online Reference

NVIDIA® maintains an official list of all GPU compute capabilities on their [CUDA™ GPUs webpage](https://developer.nvidia.com/cuda/gpus){: target="_blank"}.
