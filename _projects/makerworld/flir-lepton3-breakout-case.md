...
The Lepton3 itself is a tiny thermal imaging module, barely bigger than a fingernail, and the GroupGets breakout board that carries it is just as small and just as exposed: a bare PCB with the sensor module socketed on top and header pins along the edge. That is fine on a workbench, but the moment you want to actually use the sensor for something, mounted on a robot, pointed out of a window, carried around during testing, you need an enclosure that protects the board and the delicate sensor module without blocking its field of view.
